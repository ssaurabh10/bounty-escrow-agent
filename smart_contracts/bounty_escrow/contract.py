...
State Machine:
  ...
  OPEN | ACCEPTED → CANCELLED(9) via creator_cancel_bounty (refund creator before submit)
  ACCEPTED → OPEN via withdraw_acceptance (contributor backs out before submit)
  REJECTED → DISPUTED(5) | OPTED_OUT(8) | OPEN (reopen_after_rejection)
...
SCORE_WITHDRAW_ACCEPT = Int(5)  # lighter than opt_out (no refund to creator)
# Contributor penalty when creator cancels after someone accepted (before submit)
SCORE_CREATOR_CANCEL_ACTIVE = Int(12)  # bounty still active (before deadline)
SCORE_FAILED_TO_SUBMIT = Int(30)  # accepted but no submit before creator cancels past deadline
HASH_HEX_LEN = Int(64)
...
def post_bounty(
    ...
    return Seq(
        ...
        Assert(arbitrator_type.get() == Bytes("auto"), comment="Only automated disputes supported"),
        Assert(arbitrator_addr.get() != Global.zero_address(), comment="Oracle address required"),
...
@app.external
def withdraw_acceptance() -> Expr:
    """
    Contributor leaves after accepting but before submitting work.
    Bounty returns to OPEN; escrow stays funded; another worker may accept.
    """
    return Seq(
        assert_status(STATUS_ACCEPTED),
        assert_contributor(),
        Assert(Global.latest_timestamp() < app.state.deadline, comment="Past deadline"),
        penalize_credit_score(app.state.contributor, SCORE_WITHDRAW_ACCEPT),
        app.state.contributor.set(Global.zero_address()),
        app.state.work_ipfs_hash.set(Bytes("")),
        app.state.submission_hash.set(Bytes("")),
        app.state.submitted_at.set(Int(0)),
        app.state.status.set(STATUS_OPEN),
    )


@app.external
def creator_cancel_bounty(creator_account: abi.Account) -> Expr:
    """
    Creator closes the bounty and recovers escrow before any work is submitted.
    Allowed only in OPEN or ACCEPTED. If a worker had accepted, they are penalized
    (more severely if the bounty deadline has passed without a submission).
    """
    return Seq(
        assert_creator(),
        Assert(
        ...
        Assert(
            Or(
                app.state.status == STATUS_OPEN,
                app.state.status == STATUS_ACCEPTED,
            ),
            comment="Cannot cancel after work submitted",
        ),
        If(app.state.status == STATUS_ACCEPTED)
        .Then(
            Seq(
                Assert(
                    app.state.contributor != Global.zero_address(),
                    comment="Missing contributor",
                ),
                If(Global.latest_timestamp() < app.state.deadline)
                .Then(penalize_credit_score(app.state.contributor, SCORE_CREATOR_CANCEL_ACTIVE))
                .Else(penalize_credit_score(app.state.contributor, SCORE_FAILED_TO_SUBMIT)),
            ),
        )
        .Else(Seq()),
        ...
        app.state.submission_hash.set(Bytes("")),
        app.state.submitted_at.set(Int(0)),
        app.state.rejected_at.set(Int(0)),
        app.state.status.set(STATUS_CANCELLED),
    )
...
def submit_work(ipfs_hash: abi.String, submission_hash: abi.String) -> Expr:
    ...
    Note: Global state is world-readable. For confidentiality, store only a hash or
    ciphertext metadata on-chain; keep plaintext or decryption keys off-chain.
    """
    return Seq(
        assert_status(STATUS_ACCEPTED),
        assert_contributor(),
...
@app.external
def reopen_after_rejection() -> Expr:
    """
    Creator re-lists the bounty for a new contributor after a rejection.
    Only after the dispute window has ended (contributor did not dispute in time),
    and while the original bounty deadline has not passed. Escrow stays in the app;
    reward_amount is unchanged. Clears contributor and submission fields.
    """
    return Seq(
        assert_status(STATUS_REJECTED),
        assert_creator(),
        Assert(
            app.state.contributor != Global.zero_address(),
            comment="No assigned contributor",
        ),
        Assert(
            Global.latest_timestamp() >= app.state.rejected_at + AUTO_RELEASE_DELAY,
            comment="Dispute window still active",
        ),
        Assert(
            Global.latest_timestamp() < app.state.deadline,
            comment="Bounty deadline passed",
        ),
        app.state.contributor.set(Global.zero_address()),
        app.state.work_ipfs_hash.set(Bytes("")),
        app.state.submission_hash.set(Bytes("")),
...
def oracle_verdict(
    ...
    return Seq(
        ...
        Assert(app.state.arbitrator_type == Bytes("auto"), comment="Only automated disputes supported"),
